...
export function isEdge<Data = ElementData>(
  element: MaybeElement,
): element is Edge<Data> {
  return (
    element &&
...
export function isGraphEdge<Data = ElementData>(
  element: MaybeElement,
): element is GraphEdge<Data> {
  return isEdge(element) && 'sourceNode' in element && 'targetNode' in element;
}

export function isNode<Data = ElementData>(
  element: MaybeElement,
...
export function isGraphNode<Data = ElementData>(
  element: MaybeElement,
): element is GraphNode<Data> {
...
export function getOutgoers<N extends Node>(
  nodeOrId: Node | { id: string } | string,
  nodes: N[],
  edges: Edge[],
): N[];
export function getOutgoers<T extends Elements>(
  nodeOrId: Node | { id: string } | string,
  elements: T,
): T extends Elements ? GraphNode[] : Node[];
export function getOutgoers(...args: any[]) {
  ...
  const outgoers = elements.filter(
    (el: Element) => isEdge(el) && el.source === nodeId,
  );

  return outgoers.map((edge: Edge) =>
    elements.find((el: Element) => isNode(el) && el.id === edge.target),
  );
}

export function getIncomers<N extends Node>(
  nodeOrId: Node | { id: string } | string,
  nodes: N[],
  edges: Edge[],
): N[];
...
export function getIncomers(...args: any[]) {
  ...
  const incomers = elements.filter(
    (el: Element) => isEdge(el) && el.target === nodeId,
  );

  return incomers.map((edge: Edge) =>
    elements.find((el: Element) => isNode(el) && el.id === edge.source),
  );
}
...
export function getConnectedEdges<E extends Edge>(
...
export function getConnectedNodes<N extends Node | { id: string } | string>(
  nodes: N[],
  edges: Edge[],
) {
  const nodeIds = new Set();
...
export function getXYZPos(
  parentPos: XYZPosition,
...
export function getMarkerId(
  ...
  const idPrefix = FlowId ? `${FlowId}__` : '';
