...
>(changes: C[], elements: T[]): T[] {
  ...
  ) as (NodeAddChange | EdgeAddChange | NodeRemoveChange | EdgeRemoveChange)[];
  ...
  for (const change of addRemoveChanges) {
    if (change.type === 'add') {
      ...
      if (index === -1) {
        elements.push(change.item as unknown as T);
      }
  ...
  for (const element of elements) {
    for (const currentChange of changes) {
      ...
      switch (currentChange.type) {
        case 'select':
          (element as GraphNode | GraphEdge).selected = currentChange.selected;
          break;
        case 'position':
...
export function createAdditionChange<
  ...
  C extends NodeAddChange | EdgeAddChange = T extends GraphNode
    ? NodeAddChange
    : EdgeAddChange,
>(item: T): C {
  return <C>{
