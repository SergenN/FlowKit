...
import { ErrorCode, FlowJsError, clampPosition, isParentSelected } from '.';
...
export function getExtent<T extends NodeDragItem | GraphNode>(
  ...
  if (
    ...
    if (
      ...
      const parentExtent = getParentExtent(currentExtent, parent);

      if (parentExtent) {
        currentExtent = parentExtent;
      }
    } else {
      triggerError(new FlowJsError(ErrorCode.NODE_EXTENT_INVALID, item.id));
