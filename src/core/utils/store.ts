...
export function updateEdgeAction(
  ...
  if (!newConnection.source || !newConnection.target) {
    triggerError(new FlowJsError(ErrorCode.EDGE_INVALID, edge.id));
    return false;
  }

  if (!prevEdge) {
    triggerError(new FlowJsError(ErrorCode.EDGE_NOT_FOUND, edge.id));
    return false;
  }
...
export function createGraphNodes(
  ...
  for (let i = 0; i < nodes.length; ++i) {
    ...
    const parsed = parseNode(node, findNode(node.id), node.parentId);

    if (node.parentId) {
      parentNodes[node.parentId] = true;
    }
  ...
  for (const node of nextNodes) {
    const parentNode =
      findNode(node.parentId) || nextNodes.find((n) => n.id === node.parentId);

    if (node.parentId && !parentNode) {
      triggerError(
        new FlowJsError(ErrorCode.NODE_MISSING_PARENT, node.id, node.parentId),
      );
    }

    if (node.parentId || parentNodes[node.id]) {
 ...
 * @internal
 */
export function areConnectionMapsEqual(
...
export function createGraphEdges(
  ...
  onError: FlowJsStore['emits']['error'],
  ...
  for (const edgeOrConnection of nextEdges) {
    ...
    if (!sourceNode) {
      onError(
        new FlowJsError(ErrorCode.EDGE_SOURCE_MISSING, edge.id, edge.source),
      );
      continue;
    }
    ...
    if (!targetNode) {
      onError(
        new FlowJsError(ErrorCode.EDGE_TARGET_MISSING, edge.id, edge.target),
      );
      continue;
    }
    ...
    if (isValidConnection) {
      ...
      if (!isValid) {
        onError(new FlowJsError(ErrorCode.EDGE_INVALID, edge.id));
        continue;
      }
    }
